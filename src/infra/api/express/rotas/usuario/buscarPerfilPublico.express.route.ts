...
export class BuscarPerfilPublicoRota implements Rotas {
  ...
  public getMiddlewares(): RequestHandler[] { return [validarParamsMiddleware(idParamSchema), publicReadRateLimiter, autenticarJwtOpcional]; }
  public getHandler() {
    return async (request: Request, response: Response, next: NextFunction): Promise<void> => {
      try {
        const id = Array.isArray(request.params.id) ? request.params.id[0] : request.params.id;
        response.status(200).json(await this.buscarPerfil.executar({
          id,
          requisitanteId: request.usuario?.id,
          ...perfilPublicoQuerySchema.parse(request.query),
        }));
      } catch (error) {
