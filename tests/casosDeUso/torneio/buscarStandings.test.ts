...
describe("BuscarStandings", () => {
    ...
    it("deve incluir jogador dropado nos standings com suas estatísticas reais", async () => {
        const inscricoes = [
            new Inscricao({ id: "i1", torneioId: "t-1", usuarioId: "u-1", checkIn: true, checkInRodada: 1, dropped: false }),
    ...
    it("deve contabilizar BYE no total de partidas jogadas", async () => {
        ...
        expect(sU3.pontosMesa).toBe(3); // vitória por BYE = 3 pontos
        expect(sU3.vitoriasPartida).toBe(1);
    });

    it("deve retornar standings vazios quando não há inscrições", async () => {
        const uc = BuscarStandings.criar(
            criarMockTorneioGateway({ buscarPorId: jest.fn().mockResolvedValue(torneioAberto) }),
