...
vi.mock("bcryptjs", () => {
    const hash = vi.fn().mockResolvedValue("nova_senha_hashed");
...
describe("ConfirmarResetSenha", () => {
    it("deve redefinir a senha com token válido", async () => {
        ...
        const resetSenhaGateway = criarMockResetSenhaGateway({
            buscarPorToken: vi.fn().mockResolvedValue(criarDadosToken()),
        });
        ...
        const usuarioAtualizado = (usuarioGateway.atualizar as Mock).mock.calls[0][0] as Usuario;
    ...
    it("deve lançar erro se o token estiver expirado", async () => {
        const usuarioGateway = criarMockUsuarioGateway();
        const resetSenhaGateway = criarMockResetSenhaGateway({
            buscarPorToken: vi.fn().mockResolvedValue(
